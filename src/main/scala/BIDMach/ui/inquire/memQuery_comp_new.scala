...
object memQuery {
  ...
  def loadMemSentences_GPU(percData: Double=0.01, corpus: Int=0): (Dict, FMat, SMat, GMat, Int) =  {
    ...
    //}
    val sentsDataDir = "/big/livejournal/mercury/destress/sentences2/";
    val dictFile = sentsDataDir + "masterDict.sbmat";
    var w2vMatFile = sentsDataDir;
    if (corpus == 0) {
      w2vMatFile += "googleEmbeddings.fmat";
  ...
  def loadMemSentences_CPU(percData: Double=0.01, corpus: Int=0, seed: Int=94720): (Dict, FMat, SMat, FMat, Int, IMat, CSMat) =  {
    ...
    var dataMat = FMat(sentVecSize, totalSents);
    var sentsMat = sparse(izeros(0,0), izeros(0,0), izeros(0,0), sentsSize, 0);
	var idMat = IMat(5, totalSents);
    var userMat = CSMat(1, totalSents);
    ...
    while (iter < nrIters) {
      ...
      // sentences  -> sentsDataDir + "dataxxx_sent.smat.lz4"
      // bagOfWords -> sentsDataDir + "dataxxx.smat.lz4"
      // usernames -> sentsDataDir + "dataxxx_usernames.sbmat.lz4"
      val sentFile = sentsDataDir + currSentFile;
      val bOfwFile = sentsDataDir + currSentFile.dropRight(14)+".smat.lz4";
      val idFile = sentsDataDir + currSentFile.dropRight(14)+".imat";
      val userFile = sentsDataDir + currSentFile.dropRight(14)+"_usernames.sbmat.lz4";
      ...
      if (new File(sentFile).exists() && new File(bOfwFile).exists()) {
        ...
        val users = loadSBMat(userFile);

        // val ditemid = ids(2, 0);
        // val timestamp = ids(3, 0);
	    // val user_idx = ditemid % 100000  + 100000*(timestamp % 10000);

        val user = CSMat(users(?, 0))(0);
        ...
        if (size(data,2) == size(sents,2)) {
          ...
          userMat(0, sentsCount->(sentsCount+nrSents2Proc)) = CSMat(users(?, 0->nrSents2Proc)).t;
    ...
    return (dict, dataMat(?, 0->sentsCount), sentsMat, w2vMat, sentsCount, idMat(?, 0->sentsCount), userMat(?, 0->sentsCount))
  }
